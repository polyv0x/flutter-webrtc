...
// Public C API for registering RTCAudioProcessing::CustomProcessing
// implementations with flutter_webrtc's AudioProcessingAdapter.
//
// External Linux plugins implement RTCAudioProcessing::CustomProcessing and
// call flutter_webrtc_add_audio_processor() during their plugin init to inject
// in-band audio processing on the WebRTC capture path. The processor's
// Process() method will be called on the real-time audio thread for every
// captured audio frame, before encoding.
//
// Usage in an external plugin:
//
//   #include "flutter_webrtc/flutter_webrtc_audio_processing.h"
//
//   // In your plugin's RegisterWithRegistrar():
//   flutter_webrtc_add_audio_processor(&my_processor_instance);
//
// Requirements:
...
// Guard against redefinition when included alongside flutter_web_r_t_c_plugin.h,
// which defines the same macro.
